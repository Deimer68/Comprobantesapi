...
def parse_bancolombia_email(body: str) -> dict | None:
    ...
    monto_pattern = r"(?:valor|monto|transferencia de)[:\s]*\$?([\d.,]+)"
    remit_pattern = r"(?:de|remitente|ordenante)[:\s]+([A-Za-záéíóúÁÉÍÓÚñÑ\s]+)"
    ref_pattern   = r"(?:referencia|ref|n[uú]mero de transacci[oó]n)[:\s#]*([A-Z0-9\-]+)"
